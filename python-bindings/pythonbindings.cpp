...
PYBIND11_MODULE(blspy, m) {
    ...
    py::class_<AggregationInfo>(m, "AggregationInfo")
            .def("from_msg_hash", [](const PublicKey &pk, const py::bytes &b) {
                ...
                } catch(std::string &e){throw e;} // here and below: workaround to avoid crashes in Windows when C++
                                                  // exceptions are thrown in the called code; don't have enough time
                                                  // to analyze the root cause now
            })
